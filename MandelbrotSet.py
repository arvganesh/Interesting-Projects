...
def mandelbrot():
    ...
    npArr = np.zeros((int(w / increment), int(h / increment)), dtype=object)
    ...
    while TopLeftX <= x <= BottomRightX:
        ...
        while TopLeftY >= y >= BottomRightY:
            if not not iterate(x, y, maxIt):
                ...
                npArr[int((x - TopLeftX) / increment)][int((y - BottomRightY) / increment)] = (0,int(127.5 * iterate(x, y, maxIt)), 255 - int(127.5 * iterate(x, y, maxIt)))
